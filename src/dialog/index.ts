...
export const multiplePrompts = ({
  ...
  const api = {
    open() {
      ...
      el.addEventListener('clearvalue', (e: CustomEvent) => {
        multipleInputs?.some((v) => {
          if (v.label === e.detail?.label) {
            v?.onClear?.()
            return true
    ...
    addInput(params: MultipleInput) {
      if (!multipleInputs) return
      multipleInputs.push(params)
      el?.setMultipleInputs?.(multipleInputs)
    },
    removeInput(label: string) {
      if (!multipleInputs) return
      multipleInputs = multipleInputs.filter((v) => v.label !== label)
      el?.setMultipleInputs?.(multipleInputs)
    },
